...
class App extends Component {
  ...
  geFilteredThumbnail = () => {
    ...
    const filteredData = imagesList.filter(
      eachThumbnail => eachThumbnail.category === activeId,
    )
  ...
  render() {
    const {currentImgId, activeId} = this.state
    const filteredThumbnail = this.geFilteredThumbnail()
            ...
            <ul className="options">
              {tabsList.map(each => (
                <TabItem
                  ...
                  key={each.tabId}
                  changedActiveId={this.changedActiveId}
